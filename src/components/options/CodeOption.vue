<template>
  <div>
    <h3>Inputs</h3>
    <interface-view
      v-for="(intf, iname) in node.inputInterfaces"
      :key="iname"
      :name="iname"
      class="mt-3"
      @remove="remove(iname)"
      @rename="renameInput(iname, $event)"
    >
    </interface-view>
    ...
    <interface-view
      v-for="(intf, iname) in node.outputInterfaces"
      :key="iname"
      :name="iname"
      class="mt-3"
      @remove="remove(iname)"
      @rename="renameOutput(iname, $event)"
    >
    </interface-view>
    <button @click="addOutput" class="dark-button mt-3 w-100">
      Add Output
    </button>
  </div>
</template>
...
export default class CodeOption extends Vue {
  ...
  mounted() {
    this.node.events.addInterface.addListener(this, () => {
      this.$forceUpdate();
    });
    this.node.events.removeInterface.addListener(this, () => {
      this.$forceUpdate();
    });
  }
  ...
  addOutput() {
    this.node.addOutput(
      "Output" + Object.keys(this.node.outputInterfaces).length
    );
  }
