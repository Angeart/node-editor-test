...
export default class CustomNode extends Node {
  ...
  private defaultPoints: Vector2D[] = [
    [0, 0],
    [50, 50],
    [100, 20]
  ];
  ...
  constructor() {
    ...
    this.addInputInterface("Discrete", "CheckboxOption", true, {
      type: "boolean"
    });
    this.addOption(
      "Custom Distribution",
  ...
  public calculate() {
    ...
    const uniformRandom = this.rng!.uniform(this.state.index, {
      fixed: 8,
      min: 0,
      max: 1
    });
    const customRandom =
      this.distribution!.sample(uniformRandom) * (max - min) + min;
    this.getInterface("Output").value = discrete
      ? Math.round(customRandom)
      : customRandom;
  }
}
