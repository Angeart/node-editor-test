<template>
  <div
    ...
    style="position: relative; width: 100%; height: 70vh; max-height: 1000px;"
  >
    <canvas
      ref="customCanvas"
...
export default class CustomRandom extends Vue {
        ...
        xAxes: [
          {
            ticks: {
              ...
              callback: (label, index, labels) =>
                ...
                Math.round(
                  ((label / 100) * (this.max - this.min) + this.min) *
      ...
      tooltips: {
        callbacks: {
          label: (tooltipitem, data) => {
            ...
            const xLabel: string = tooltipitem.xLabel
            ...
            label +=
              Math.round(
                ((parseFloat(xLabel) / 100) * (this.max - this.min) +
                  this.min) *
                  Math.pow(10, this.digits)
              ) / Math.pow(10, this.digits);
            label += ")";
  ...
  mouseDownHandler(e: MouseEvent) {
    ...
    if (e.button === 2) {
      ...
      if (firstPoint) {
        ...
        if (
          selectedPoint !== this.startPoint &&
          selectedPoint !== this.endPoint
        ) {
  ...
  mouseMoveHandler(e: MouseEvent) {
    ...
    if (this.draggedPoint) {
      ...
      if (
        this.draggedPoint === this.startPoint ||
        this.draggedPoint === this.endPoint
      ) {
  ...
  get areaColor() {
    const rgba = new RegExp(/rgba\((\d*),(\d*),(\d*),(\d*)\)/, "gi").exec(
      this.curveColor
    );
    if (rgba) {
      return (
        "rgba(" + rgba[1] + "," + rgba[2] + "," + rgba[3] + "," + 0.1 + ")"
      );
    } else {
